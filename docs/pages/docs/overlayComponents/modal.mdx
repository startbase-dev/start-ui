...
import GlobalModalExample from '@components/ModalExample/ModalExample';
...
<Tabs items={['npm', 'yarn', 'pnpm', 'bun']}>
  ...
  <Tabs.Tab>
    ```bash copy
    pnmp add @start-base/react-modal
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```bash copy
    bun add @start-base/react-modal
    ```
  </Tabs.Tab>
</Tabs>
...
<Tabs items={['Individual', 'Global']}>
  <Tabs.Tab>
    ```jsx copy
    import Modal from '@start-base/react-modal/Modal';
    import GlobalModals from '@start-base/react-modal/GlobalModals';
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```jsx copy
    import { Modal, GlobalModals } from '@start-base/react-modal';
    ```
  </Tabs.Tab>
</Tabs>
...
function MyComponent() {
  ...
  return (
    <button
      onClick={() => openModal('modal-1', { callback: handleDelete })}
    >
      Open modal-1
    </button>
  );
